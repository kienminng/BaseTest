...
public class ProductService : IProductService
{
    ...
    {
        var query = _productRepo.GetQueryable(record => !record.IsDeleted && record.Status);
    ...
    public async Task Delete(int id)
    {
        ...
        if (product is null) throw new Exception(ErrorMessage.ProductErrorMessage.ProductNotExisted);
    ...
    {
        if (!input.Name.IsNullOrEmpty()) query = query.Where(record => record.Name.Contains(input.Name));

        if (input.StartPrice > input.EndPrice) throw new Exception();
        ...
        if (input.Discount >= 0) query = query.Where(record => record.Discount == input.Discount);
    ...
    private async void ValidateProductCreate(ProductCreatingRequest request)
    {
        ...
        if (request.Price <= 0) throw new Exception(ErrorMessage.ProductErrorMessage.PriceError);

        if (request.Discount < 0) throw new Exception(ErrorMessage.ProductErrorMessage.Discount);
    }
}